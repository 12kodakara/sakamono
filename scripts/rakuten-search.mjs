 ...
 * 楽天市場検索コマンドの入口。
 *
 *   npm run data:rakuten:search -- --product=<id> --sample
 *   npm run data:rakuten:search -- --product=<id> --dry-run
 *
 * 中身は TypeScript（src/data/external/rakuten/cli.ts）です。
...
const outFile = path.join(outDir, 'rakuten-cli.mjs')
...
async function run() {
  ...
  const result = await build({
    entryPoints: [path.join(projectRoot, 'src/data/external/rakuten/cli.ts')],
    bundle: true,
    platform: 'node',
    format: 'esm',
    target: 'node20',
