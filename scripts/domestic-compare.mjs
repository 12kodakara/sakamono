 ...
 *
 * 中身は TypeScript（src/data/external/domestic/compareCli.ts）です。
...
const outFile = path.join(outDir, 'domestic-compare-cli.mjs')
...
async function run() {
  ...
  const result = await build({
    entryPoints: [path.join(projectRoot, 'src/data/external/domestic/compareCli.ts')],
    bundle: true,
    platform: 'node',
    format: 'esm',
    target: 'node20',
